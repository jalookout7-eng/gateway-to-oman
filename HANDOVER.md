...
2. **Refactor `lib/email/sender.ts:sendEmail()` to use the settings-DB provider path (deferred — JA's call 2026-05-27).** Code review found two email paths in the codebase: (a) `sendOtpEmail()` + `sendEmailLegacy()` correctly read provider config from the `settings` table (so they go through Resend automatically once configured); (b) `sendEmail()` — used by `/api/admin/emails/[id]/send` for **booking confirmations** and the **admin-approved lead/follow-up email "Send" button on `/admin/leads`** — still reads raw `EMAIL_HOST / EMAIL_PORT / EMAIL_USER / EMAIL_PASS / EMAIL_FROM_ADDRESS` env vars (legacy from Phase 5). Those env vars are **not set in Vercel**, so admin-approved emails will silently fail when Ahmed tries to send them. **Recommended fix:** refactor `sendEmail()` to use the same settings-DB-driven provider routing as the OTP path (one source of truth, no extra env vars, future-proof on provider changes). Estimated ~30 min code + a couple of tests. **Scope:** keep `sendEmail()` signature stable so the callsites at `/api/admin/emails/[id]/send` don't need to change; internally route to Resend (or whichever provider is configured in `settings`). **Workaround in the meantime:** if Ahmed needs to send a follow-up email before this fix lands, he can copy the draft body from `/admin/leads → expand lead → Email Pending Approval` and send manually from his regular mailbox.
...
12. **Remaining security items (lower priority):** M-2 Google `id_token` signature/claims verification (becomes relevant once OAuth lives); `npm audit fix` (dev-only vulns — run separately); L-1 SameSite-strict admin cookie; L-5 mask email creds in admin UI; I-4 `cover_image_url` validation. See §14.
