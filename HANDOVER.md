...
## 11. Status Snapshot — Completed / Pending (2026-05-26)
...
**Active configuration note:** AI chat currently runs on **Groq Llama 3.3 70B** (`AI_PROVIDER=groq` temp override in Vercel) because the Anthropic API key isn't set yet. Once `ANTHROPIC_API_KEY` is added in Vercel and the temp override removed, the code automatically flips to Anthropic Haiku 4.5 primary with Groq failover (no redeploy of code needed — just a Vercel redeploy to pick up the env change). Walkthrough sent to JA.

**🐛 Known issues found post-deploy (2026-05-26) — to fix in Batch 5:**
1. **R2 image rendering breaks on the frontend.** Uploads land in the `gto-listings` R2 bucket successfully, but the rendered listing card shows a broken-image icon (see `Cloudflare err.png` in client root). Likely cause: `next.config.js` `remotePatterns` wildcard format (`*.r2.dev`) may be incompatible with Next 14.2.x's stricter parsing, OR the stored `cover_image_url` URL diverges from the actual reachable R2.dev URL. **Probe steps:** open the stored URL directly in a browser (does it serve?); verify `R2_PUBLIC_BASE_URL` in Vercel matches the bucket's R2.dev URL exactly; confirm `next.config.js` accepts `pub-<hash>.r2.dev`.
2. **AI summary on `/admin/leads` — formatting + identity bug** (see `Leads section - admin.png`). (a) The summary returns markdown (`**WHO:**` etc.) but is rendered as plain text → users see raw asterisks. (b) For a placeholder lead with no real conversation, the AI hallucinates "Ahmed Al-Azizi" (the GTO owner) as the lead's name — i.e. the summarize prompt is leaking owner identity from the chatbot KB context. **Fix:** either render markdown or instruct the prompt to emit plain text; scope the summarize prompt to the lead's actual data only (no KB / owner identity); short-circuit with "No conversation captured" when history is empty.
3. **Admin-editable status & segment options.** `leads.status` has a SQL CHECK constraint (5 fixed values) and `leads.segment` has another (4 fixed values). Ahmed wants to add custom values via admin. **Needs:** SQLite table-rebuild migration to drop the two CHECK constraints (SQLite doesn't support `DROP CONSTRAINT` — requires a new-table + copy + rename); new allowed-values lists in `settings`; admin UI under `/admin/settings`; dynamic validation in the `PATCH /api/admin/leads/[id]` route reading from settings.
4. **Mobile admin logout missing.** Desktop sidebar has a Sign Out button (`app/admin/layout.tsx`); mobile bottom-nav doesn't surface it. Add a visible logout (header icon on mobile, or as a 6th bottom-nav item).
5. **Other errors TBD** — JA actively testing; more issues will be added here as they surface.

**Pending / open:**
- **Anthropic primary setup** — Ahmed/JA to create account at console.anthropic.com (recommend `gatewaytooman@gmail.com` for ownership consistency), add card, create API key, paste into Vercel as `ANTHROPIC_API_KEY`, remove the temp `AI_PROVIDER=groq` override, redeploy. Once done: Haiku 4.5 primary, Groq failover. Walkthrough provided.
- **Production domain — point `gatewaytooman.com` to Vercel via GoDaddy DNS.** Site is currently only reachable at `gateway-to-oman.vercel.app`. To switch to the proper apex (+ www): **Vercel → Project → Settings → Domains → Add `gatewaytooman.com` and `www.gatewaytooman.com`**; Vercel will display the exact DNS records to add (typically `A @ → 76.76.21.21` and `CNAME www → cname.vercel-dns.com`); add those in **GoDaddy → Domain → DNS**; Vercel auto-verifies and provisions SSL within minutes. Independent of the R2 custom-domain DNS-migration item below (different subdomain, can stay on GoDaddy DNS for this).
- **Compliance — Ahmed + lawyer review before going public-facing.** The current `/privacy`, `/terms`, `/cookies` pages carry full final-form wording (no DRAFT watermark) but must be reviewed with **Ahmed** for business-accuracy (sub-processors used, retention practices, etc.) AND a **qualified privacy lawyer** for Oman PDPL + GDPR conformance before relying on them. Treat as draft for compliance purposes until that review is complete. Removed the internal `gto_admin_session` cookie from the public Cookie Notice 2026-05-26 — it's an internal admin cookie and not user-relevant.
- **Compliance company-fact fill-ins** — `components/legal/CompanyFacts.ts` has 4 `[bracketed]` placeholders pending from Ahmed (legal entity name · CR number · Oman address · privacy contact email).
- **Remaining security items (lower priority):** M-2 Google `id_token` signature/claims verification (Google OAuth not live yet — needs credentials); `npm audit fix` (dev-only vulns — run separately); L-1 SameSite-strict admin cookie; L-5 mask email creds in admin UI; I-4 `cover_image_url` validation. See §14.
- **DNS migration GoDaddy → Cloudflare** (optional polish) — to swap the R2.dev subdomain (`pub-…r2.dev`) for `media.gatewaytooman.com`. Single env var change + uncomment a `next.config.js` line after DNS lands.
- **Multiple-choice / quick-reply answer UI** (Omar "piece F") — deferred to its own spec/plan.
- **Other brain-dump fixes still open**: sign-up phone country-code field width; OTP not sending (depends on email-provider configuration); booking calendar mobile + Calendly sync; security stress-test + scaling to ~1000 concurrent; confirm AI lead summary on click. (Poppins font change — declined.)
- **Meeting-notes audit** (Thread 2 item 3) — not started.
- **Repo hygiene:** `master` is now ~80 commits behind `section-b-marketplace`; production deploys from `section-b`, so this is cosmetic.
