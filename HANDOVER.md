...
**Pending / open:**
- **Compliance & privacy (Batch 3)** — design APPROVED 2026-05-26. **BUILT on `section-b-marketplace` (Batch 3 — 2026-05-26).** Scope shipped: `/privacy`, `/terms`, `/cookies` (static, server-rendered, full final wording from spec); footer links on main site + `/businesses`; required marketplace sign-up consent checkbox (gates submit); passive consent lines on `LeadCaptureForm` + `AccessRequestForm`. Bundles into the same pending deploy as Batch 1 + Batch 2 — **same deploy order applies:** `npm run migrate` first (Batch 2 IV2 columns + rate_limits table), then `vercel deploy --prod`. No migration required specifically for compliance pages. Company-specific facts in `components/legal/CompanyFacts.ts` — four `[bracketed]` fill-ins still pending from Ahmed (legal entity name · CR number · Oman address · privacy contact email). See §12.
